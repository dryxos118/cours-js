...
const input = document.querySelector(".form-center");
...
input.addEventListener("submit", function (e) {
  ...
  //premiere condition
  if (num <= 0 || num > 9) {
    //si il est inferieur ou egal a 0 ou superieur a 9
    txt.innerHTML = `<p class="showTxt">${text[random]}</p>`;
  } else {
    //si le numbre est entre 1 et 9
    let Texte = text.slice(0, num);
    Texte = Texte.map(function (item) {
      return `<p class="showTxt">${item}</p>`;
    }).join("");
    txt.innerHTML = Texte;
  }
});
